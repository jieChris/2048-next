...
test.describe("Legacy Multi-Page Smoke", () => {
  test("guest login panel uses the approved compact centered layout", async ({
    page,
  }) => {
    await page.addInitScript(() =>
      window.localStorage.setItem("theme_profile_v1", "mist_cyan"),
    );
    await page.goto("/account_settings.html", {
    ...
    const outerStackStyle = await page
      .locator(".settings-stack")
      .evaluate((stack) => {
    ...
    const authSectionStyle = await page
      .locator(".settings-auth-section")
      .evaluate((section) => {
    ...
    await expect(page.locator("#account-auth-heading")).toHaveCSS(
      "font-size",
      "25px",
    );
    await expect(page.locator("#account-auth-state-tag")).toHaveCSS(
      "padding-left",
      "12px",
    );
    await expect(page.locator("#account-auth-state-tag")).toHaveCSS(
    ...
    await expect(page.locator(".account-auth-form-surface")).toHaveCSS(
      "padding-top",
      "19px",
    );
    await expect(page.locator(".account-auth-form-surface")).toHaveCSS(
    ...
    await expect(
      page.locator(".account-auth-form-surface > #account-action-row"),
    ).toHaveCount(1);
    await expect(page.locator("#account-action-row")).toHaveCSS(
      "background-color",
      "rgba(0, 0, 0, 0)",
    );
    await expect(page.locator("#account-action-row")).toHaveCSS(
    ...
    await expect(
      page.locator('link[href^="style/account_settings_page.css"]'),
    ).toHaveAttribute(
      "href",
      "style/account_settings_page.css?v=20260827-mobile-initial-state-v1",
    );

    const alignment = await page
      .locator(".settings-auth-section")
      .evaluate((section) => {
        const form = section.querySelector<HTMLElement>(
          ".account-auth-form-surface",
        )!;
  ...
  test("guest login panel stays usable on narrow mobile screens", async ({
    page,
  }) => {
    await page.addInitScript(() =>
      window.localStorage.setItem("theme_profile_v1", "mist_cyan"),
  ...
  test("account settings page supports nickname/password/logout flows", async ({
    page,
  }) => {
    ...
    const response = await page.goto("/account_settings.html", {
      waitUntil: "domcontentloaded",
    });
    expect(response, "Account settings response should exist").not.toBeNull();
    expect(
      response?.ok(),
      "Account settings response should be 2xx",
    ).toBeTruthy();
    ...
    await expect(page.locator("#settings-current-nickname")).toHaveText(
      "SmokeUser",
    );
    await expect(page.locator("#settings-nav-user")).toBeVisible();
    await expect(page.locator("#settings-nav-user")).toHaveAttribute(
      "href",
      "user.html",
    );

    const centeredPanels = await page
      .locator(".settings-stack")
      .evaluate((card) => {
        ...
        const panels = [
          ...
          ...Array.from(
            card.querySelectorAll<HTMLElement>("[data-settings-account-only]"),
          ),
        ...
        return panels.map((panel) => {
          ...
          return {
            centerDelta: Math.abs(
              cardRect.left + cardRect.width / 2 - (rect.left + rect.width / 2),
            ),
            width: rect.width,
          };
        });
    ...
    expect(
      centeredPanels.every((panel) => panel.centerDelta <= 1),
    ).toBeTruthy();
    ...
    await expect(page.locator("#settings-new-nickname")).toHaveClass(
      /input-error/,
    );
    await expect(page.locator("#settings-nickname-feedback")).toContainText(
      "Nickname unavailable",
    );

    await page.fill("#settings-new-nickname", "SmokeNew");
    await expect(page.locator("#settings-new-nickname")).not.toHaveClass(
      /input-error/,
    );
    ...
    await expect
      .poll(() =>
        page.evaluate(
          () =>
            (window as Window & { __nicknameConfirmMessages?: string[] })
              .__nicknameConfirmMessages?.[0] || "",
        ),
      )
    ...
    await page.waitForFunction(
      () => window.localStorage.getItem("2048_auth_nickname_v1") === "SmokeNew",
    );
