...
test.describe("Legacy Multi-Page Smoke", () => {
  ...
  test("index ui no longer exposes retired guide runtime contracts", async ({ page }) => {
    const response = await page.goto("/2048.html", {
      waitUntil: "domcontentloaded"
    });
    expect(response, "Index response should exist").not.toBeNull();
    ...
    expect(snapshot.hasRuntime).toBe(true);
    expect(snapshot.hasGuideContractResolver).toBe(false);
    expect(snapshot.hasGuideRuntime).toBe(false);
  });
