...
describe("core game manager saved state persistence binding", () => {
  it("no-ops when persistence binding preconditions are missing", () => {
    ...
    bindGameManagerSavedStatePersistence({ getWindowLike: () => null }, operations);
    bindGameManagerSavedStatePersistence(
  ...
  it("binds page lifecycle persistence once and saves forced state on page exit", () => {
    ...
    expect(windowLike.addEventListener).toHaveBeenNthCalledWith(2, "pagehide", expect.any(Function));
    expect(operations.bindSavedStateSyncStorageListener).toHaveBeenCalledWith(manager, windowLike);
    ...
    expect(windowLike.OnlineLeaderboardRuntime.persistRankedCheckpointOnPageHide).toHaveBeenCalledWith(
      manager
    );
  });
  ...
  it("creates and installs the legacy runtime shape without replacing an existing runtime", () => {
    ...
    expect(installGameManagerSavedStatePersistenceBindingRuntime({ windowLike })).toBe(
    ...
    expect(
      installGameManagerSavedStatePersistenceBindingRuntime({
        windowLike: { CoreGameManagerSavedStatePersistenceBindingRuntime: existing }
      })
    ).toBe(existing);
  });
});
