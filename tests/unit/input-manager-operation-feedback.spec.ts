...
function loadInputManager(fileName: string, constructorName: string) {
  ...
  vm.runInNewContext(
    readFileSync(path.resolve(process.cwd(), "js", fileName), "utf8"),
    context,
  );
...
describe("input manager operation feedback metadata", () => {
  it.each([
    ...
    expect(payloads[1]).toEqual({
      direction: 1,
      feedback: { id: expect.any(String), key: "swipe-right", repeat: false },
    });
  });
  ...
  it("uses classic undo/redo semantics and stable keyboard tokens", () => {
    const { dom, inputManager } = loadInputManager(
      "keyboard_input_manager.js",
      "KeyboardInputManager",
    );
    const payloads: Array<{
      direction: number;
      feedback: { id: string; key: string };
    }> = [];
    inputManager.on("move", (payload) =>
      payloads.push(payload as (typeof payloads)[number]),
    );
    ...
    expect(
      payloads.map(({ direction, feedback }) => [direction, feedback.key]),
    ).toEqual([
      [0, "W"],
      [-1, "Z"],
      [-1, "U"],
    ...
    expect(new Set(payloads.map((payload) => payload.feedback.id)).size).toBe(
      payloads.length,
    );
  });
  ...
  it("does not create feedback attempts for editable text, items, restart, or capped Backspace", () => {
    const keyboard = loadInputManager(
      "keyboard_input_manager.js",
      "KeyboardInputManager",
    );
    const capped = loadInputManager(
      "capped_input_manager.js",
      "CappedInputManager",
    );
    const keyboardPayloads: unknown[] = [];
    const cappedPayloads: unknown[] = [];
    keyboard.inputManager.on("move", (payload) =>
      keyboardPayloads.push(payload),
    );
