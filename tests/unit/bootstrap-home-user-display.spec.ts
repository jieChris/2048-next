...
describe("bootstrap home user display", () => {
  ...
  it("links guests to the account settings login page", () => {
    expect(resolveHomeUserDisplayHref({ storageLike: null })).toBe(
  ...
  it("links signed-in users to their profile page", () => {
    ...
    expect(resolveHomeUserDisplayHref({ storageLike })).toBe(
      "user.html?id=9&nickname=Smoke+User",
    );
  });
  ...
  it("syncs the top profile button href with the same destination", () => {
    ...
    expect(label.href).toBe(
      "user.html?id=12&nickname=Codex+%E6%B5%8B%E8%AF%95",
    );
  ...
  it("does not create a profile button on the practice board", () => {
    ...
    expect(
      syncHomeUserDisplay({ documentLike, storageLike, pageId: "practice" }),
    ).toBe(false);
  ...
  it("creates a global label for regular pages without an existing node", () => {
    ...
    expect(
      bindHomeUserDisplay({ documentLike, storageLike, pageId: "relay-5x5" }),
    ).toBe(true);
  ...
  it("creates a heading-aligned label for game pages with a heading container", () => {
    ...
    expect(
      bindHomeUserDisplay({ documentLike, storageLike, pageId: "play" }),
    ).toBe(true);
  ...
  it("does not create a global label on excluded utility pages and hub pages", () => {
    ...
    expect(
      bindHomeUserDisplay({ documentLike, storageLike: null, pageId: "modes" }),
    ).toBe(false);
    expect(
      bindHomeUserDisplay({
        documentLike,
