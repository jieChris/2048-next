...
describe("self-hosted nginx cache policy", () => {
  it("serves immutable static assets and short-cached HTML without no-store", () => {
    ...
    expect(config).not.toContain("no-cache");
    expect(config).toMatch(/location\s+~\*\s+\\\.\(\?:js\|mjs\|css[\s\S]*?max-age=31536000,\s*immutable/);
