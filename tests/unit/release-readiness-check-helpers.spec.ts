...
describe("release-readiness-check helpers", () => {
  ...
  it("ensures required snippets exist", () => {
    expect(() =>
      ensureContainsSnippets("alpha beta", ["alpha"], "demo"),
    ).not.toThrow();
    expect(() =>
      ensureContainsSnippets("alpha beta", ["missing"], "demo"),
    ).toThrow(/demo missing required snippet/);
  });
  ...
  it("extracts workflow job blocks and validates needs dependencies", () => {
    ...
    const diagnosticsBlock = extractWorkflowJobBlock(
      workflow,
      "diagnostics-index",
    );
    ...
    expect(() =>
      ensureJobNeedsDependency(workflow, "release-ready", "smoke"),
  ...
  it("validates smoke workflow sharding + diagnostics topology contract", () => {
    const workflow = [
      ...
      '      REFACTOR_GATE_TIMEOUT_CORE_PERFORMANCE_MS: "900000"',
      "      CORE_PERFORMANCE_BASELINE_REF: base-sha",
      ...
      "          artifacts/core-performance/latest.json",
    ...
    expect(() =>
      verifySmokeWorkflowShardingContent(
        workflow.replace("Timeout tuning key(s):", "MISSING"),
      ),
  ...
  it("validates refactor-gate timeout and smoke-script contract", () => {
    const gate = [
      ...
      'const performance = { name: "core-performance", cmd: "npm", args: ["run", "test:performance:core"] };',
