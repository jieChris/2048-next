...
function loadMoveInputRuntime(
  ...
  const executeUndoMove = vi.fn(
    () => options.undoResult || { handled: false, valid: false },
  );
  const context = {
    console,
    Date: RuntimeDate,
...
describe("core game manager move input runtime", () => {
  it("publishes the exact immediate attempt with validity from manager.move", () => {
    const { runtime, publishConfirmedOperationFeedback } =
      loadMoveInputRuntime();
    const attempt = createAttempt("key-1");
    const manager = { move: vi.fn(() => true), lastMoveInputAt: 0 };

    expect(runtime.executeImmediateMoveInput(manager, attempt, 1_234)).toBe(
      true,
    );
  ...
  it("stops publishing inputs that begin after game termination while preserving undo", () => {
    ...
    expect(
      runtime.executeImmediateMoveInput(
        manager,
        createAttempt("key-after-over"),
        1_234,
      ),
    ).toBe(false);
    expect(manager.move).toHaveBeenCalledWith(0);
    ...
    expect(
      runtime.executeImmediateMoveInput(
        manager,
        createAttempt("key-rescue-undo", -1),
        1_235,
      ),
    ).toBe(true);
  ...
  it("keeps throttled attempts in order and publishes each execution", () => {
    const { runtime, publishConfirmedOperationFeedback, setNow } =
      loadMoveInputRuntime({
        throttleMs: 50,
      });
    ...
    const manager = {
      ...
      pendingMoveInputQueue: [] as MoveAttempt[],
    ...
    expect(manager.pendingMoveInputQueue).toEqual([first, latest]);
    ...
    expect(manager.move.mock.calls).toEqual([[0], [1]]);
    expect(publishConfirmedOperationFeedback.mock.calls).toEqual([
      [manager, first, true],
      [manager, latest, true],
    ]);
  });

  it("executes an already delayed queue in order when newer attempts arrive", () => {
    const {
      runtime,
      delayedCallbacks,
      publishConfirmedOperationFeedback,
    ...
    animationFrames.shift()!();
    expect(manager.move).toHaveBeenCalledWith(1);
    expect(publishConfirmedOperationFeedback).toHaveBeenCalledTimes(2);
  });
  ...
  it("returns booleans for successful, unmoved, and restored moves", () => {
    ...
    successful.context.buildMovePlan = vi.fn(() => ({
      vector: { x: 0, y: -1 },
    }));
    ...
    const undo = loadMoveInputRuntime({
      undoResult: { handled: true, valid: true },
    });
    expect(undo.runtime.move(manager, -1)).toBe(true);
    expect(undo.executeUndoMove).toHaveBeenCalledWith(
      manager,
      -1,
      expect.any(Object),
    );
