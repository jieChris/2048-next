...
describe("core saved-state sync payload", () => {
  it("builds lightweight trim fields for cross-tab sync", () => {
    ...
    expect(
      buildSavedStateSyncTrimPayload({ ipsInputCount: -1 }).ips_input_count,
    ).toBe(0);
  });
  ...
  it("parses valid cross-tab saved-state events and rejects malformed payloads", () => {
    ...
    expect(
      parseSavedStateSyncEventPayload(JSON.stringify({ state: [] })),
    ).toBeNull();
    expect(
      parseSavedStateSyncEventPayload(JSON.stringify({ state: {} })),
    ).toBeNull();
  ...
  it("creates and installs the legacy runtime shape without replacing an existing runtime", () => {
    ...
    expect(runtime.buildSavedStateSyncTrimPayload).toBe(
      buildSavedStateSyncTrimPayload,
    );
    expect(runtime.parseSavedStateSyncEventPayload).toBe(
      parseSavedStateSyncEventPayload,
    );
    ...
    expect(
      windowLike.CoreSavedStateSyncPayloadRuntime
        ?.buildSavedStateSyncTrimPayload,
    ).toBe(buildSavedStateSyncTrimPayload);
    expect(
      windowLike.CoreSavedStateSyncPayloadRuntime
        ?.parseSavedStateSyncEventPayload,
    ).toBe(parseSavedStateSyncEventPayload);

    const existing = {
      buildSavedStateSyncTrimPayload: vi.fn(),
      parseSavedStateSyncEventPayload: vi.fn(),
    };
    expect(
      installSavedStateSyncPayloadRuntime({
