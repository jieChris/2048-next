...
describe("user-profile entry bootstrap", () => {
  ...
  it("keeps the open profile synchronized with display-mode changes", () => {
    ...
    expect(page).toContain('import { bindDisplayModeSync } from "../bootstrap/display-mode";');
    expect(page).toContain("bindDisplayModeSync({ documentLike: document, windowLike: window });");
  });
  ...
  it("keeps the tide cover as one coherent scene with reduced-motion support", () => {
    ...
    expect(css).toContain('.user-profile-cover:not([data-background-assets-ready]) .user-profile-cover-art-sky');
  ...
  it("shows the 150-code-point bio editor only in the owner's edit mode", () => {
    ...
    expect(html).toContain('id="user-profile-bio-status" aria-live="polite"');
    expect(html).toContain('id="user-profile-moderation-history"');
    expect(page).toContain('safeReadStorageItem');
    expect(page).toContain('profileValue(profile, "profile_bio", "profileBio")');
  ...
  it("submits bio moderation without optimistically replacing the public bio", () => {
    const page = readEntry("src/pages/user-profile-page.ts");
    const save = page.slice(page.indexOf("async function saveProfileBio"), page.indexOf("function applyLanguage"));
    const guardIndex = save.indexOf("if (bioSaving) return;");

    expect(guardIndex).toBeGreaterThan(-1);
    expect(guardIndex).toBeLessThan(save.indexOf('randomId("profile-bio", 16)'));
    expect(guardIndex).toBeLessThan(save.indexOf('apiClient.request("/user/me/profile"'));
    expect(save).toContain('apiClient.request("/user/me/profile"');
    expect(save).toContain('"Idempotency-Key": randomId("profile-bio", 16)');
    expect(save).toContain("body: JSON.stringify({ profile_bio: profileBio, revision })");
  ...
  it("renders owner-only moderation history through safe localized status and reason maps", () => {
    ...
    expect(page).toContain('apiClient.request("/user/me/moderation-submissions?limit=20"');
    for (const status of ["submitted", "ai_reviewing", "ai_pass", "ai_reject", "manual_review", "failed_retryable", "approved", "rejected"]) {
  ...
  it("uploads one reviewed avatar only from the owner's edit mode without replacing the public avatar optimistically", () => {
    const html = readEntry("user.html");
    const page = readEntry("src/pages/user-profile-page.ts");
    const avatarFlow = page.slice(page.indexOf("function avatarNoticeCopy"), page.indexOf("function availableFeaturedModes"));
    ...
    expect(html).toContain('id="user-profile-avatar-status" aria-live="polite"');
    expect(page).toContain("avatarEditor.hidden = !(canEdit && editModeEnabled)");
    expect(avatarFlow).toContain('apiClient.request("/user/me/avatar-submission", { method: "GET" })');
    expect(avatarFlow).toContain("file.size > 200 * 1024");
    expect(avatarFlow).toContain('form.set("avatar", file)');
    expect(avatarFlow).toContain('headers: { "Idempotency-Key": randomId("profile-avatar", 16) }');
  ...
  it("loads one published scene and switches its three layers with the display mode", () => {
    ...
    expect(page).toContain('`/profile-backgrounds/${encodeURIComponent(sceneId || "default")}/layers?variant=${variant}`');
    ...
    expect(page).toContain('backgroundEditor.hidden = !(canEdit && editModeEnabled)');
    expect(page).toContain('body: JSON.stringify({ background_scene_id: selectedSceneId, revision })');
  ...
  it("fails closed when the authenticated ownership lookup is unavailable", () => {
    ...
    const init = legacy.slice(legacy.indexOf("async function init()"), legacy.indexOf("global.UserProfilePageRuntime"));
