...
async function readHomeStandardBundle(
        ...
        return {
                ...
                async closeBundle() {
                        ...
                        await writeFile(
                                resolve(
                                        targetDir,
                                        HOME_STANDARD_STARTUP_BUNDLE,
                                ),
                                await readHomeStandardBundle(
                                        HOME_STANDARD_STARTUP_FILES,
                                ),
                                "utf8",
                        );
                        await writeFile(
                                resolve(
                                        targetDir,
                                        HOME_STANDARD_DEFERRED_BUNDLE,
                                ),
                                await readHomeStandardBundle(
                                        HOME_STANDARD_DEFERRED_FILES,
                                ),
                                "utf8",
                        );
                },
                configureServer(server) {
                        server.middlewares.use(async (req, res, next) => {
                                const pathName = req.url
                                        ? req.url.split("?")[0]
                                        : "";
                                const bundleFiles: Record<
                                        string,
                                        readonly string[]
                                > = {
                                        [`/js/${HOME_STANDARD_STARTUP_BUNDLE}`]:
                                                HOME_STANDARD_STARTUP_FILES,
                                        [`/js/${HOME_STANDARD_DEFERRED_BUNDLE}`]:
                                                HOME_STANDARD_DEFERRED_FILES,
                                };
...
export default defineConfig(({ mode }) => {
        ...
        return {
                base: "./",
                plugins: [
                        copyRootLegacyScriptsPlugin(),
                        copyOpenApiContractPlugin(),
                ],
                server: {
                        proxy: apiProxy,
                },
                ...
                build: {
                        ...
                        rollupOptions: {
                                input: {
                                        ...
                                        leaderboard_4x4: resolve(
                                                __dirname,
                                                "leaderboard_4x4.html",
                                        ),
                                        admin: resolve(__dirname, "admin.html"),
                                        account_settings: resolve(
                                                __dirname,
                                        ...
                                        visual_preview: resolve(
                                                __dirname,
                                                "visual-preview.html",
                                        ),
                                        operation_feedback_preview: resolve(
                                                __dirname,
                                                "operation-feedback-preview.html",
                                        ),
                                        api_docs: resolve(
                                                __dirname,
                                                "api-docs.html",
