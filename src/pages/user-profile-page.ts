...
type UserProfileWindow = Window & {
  UserProfilePageRuntime?: LegacyProfileRuntime;
};

const PROFILE_COVERS = new Set([
  "tide",
  "sunset",
  "midnight",
  "forest",
...
const ACTIVE_MODERATION_STATUSES = new Set([
  "submitted",
  "ai_reviewing",
  "ai_pass",
  "manual_review",
...
const SAFE_MODERATION_REASON_COPY: Record<string, [string, string]> = {
  ...
  moderation_sla_exceeded: [
    "自动审核超时，已转人工复核",
    "Automated review timed out and was sent to manual review",
  ],
  policy_version_mismatch: [
    "审核策略已更新，等待人工复核",
    "The review policy changed; awaiting manual review",
  ],
  provider_disabled: [
    "自动审核暂不可用，等待人工复核",
    "Automated review is unavailable; awaiting manual review",
  ],
  ...
  provider_budget_exceeded: [
    "自动审核繁忙，等待人工复核",
    "Automated review is busy; awaiting manual review",
  ],
  provider_timeout: ["审核服务响应超时", "The review service timed out"],
  provider_rate_limited: [
    "审核服务繁忙，等待重试",
    "The review service is busy; awaiting retry",
  ],
  provider_invalid_response: [
    "审核结果异常，等待复核",
    "The review result was invalid; awaiting review",
  ],
...
let bioBlockedUntil = "";
let bioSaving = false;
let bioNotice: { key: string; value?: string; kind?: "error" | "ok" } | null =
  null;
...
function language(): "zh" | "en" {
  ...
  return String(
    safeReadStorageItem({ storageLike, key: "ui_language_v1" }) || "",
  )
    .toLowerCase()
    .startsWith("en")
    ? "en"
...
function profileValue(
  ...
  return (
    (profile[snake] as ProfileValue | undefined) ??
    (profile[camel] as ProfileValue | undefined)
  );
}
...
function modeLabel(modeKey: unknown, modeBucket?: unknown): string {
  const runtimeLabel = legacyRuntime().formatModeLabel?.(
    text(modeKey),
    text(modeBucket),
  );
...
function setAvatar(
  ...
  image.src = avatarUrl;
  image.alt =
    language() === "en" ? `${nickname}'s avatar` : `${nickname}的头像`;
...
function joinedText(createdAt: unknown): string {
  ...
  if (!Number.isFinite(timestamp))
    return language() === "en" ? "Joined 2048 Next" : "加入 2048 Next";
  const days = Math.max(
    1,
    Math.floor((Date.now() - timestamp) / 86_400_000) + 1,
  );
  return language() === "en"
    ? `Day ${days} on 2048 Next`
    : `加入 2048 Next 第 ${days} 天`;
}
...
function normalizeState(value: unknown): LegacyProfileState {
  ...
  return {
    ...
    stats: {
      ...
      tileMilestones: records(stats.tileMilestones),
    },
...
function syncLegacyState(raw: unknown): void {
  ...
  state = next;
  if (filtersAreOverviewDefault(next.filters))
    recentRecords = next.records.slice(0, 3);
  renderProfile();
  renderStatsContent();
  if (next.targetUserId >= 0 && achievementsUserId !== next.targetUserId)
    void loadAchievements(next.targetUserId);
}
...
function resetProfileBackgroundLayers(
  cover = byId<HTMLElement>("user-profile-cover"),
): void {
  PROFILE_BACKGROUND_IMAGE_PROPERTIES.forEach((name) =>
...
async function loadProfileBackgroundLayers(sceneId: string): Promise<void> {
  ...
  const loaded = await Promise.all(
    values.map(([, value]) => preloadProfileBackground(value)),
  );
  if (renderedBackgroundKey !== key) return;
  ...
  values.forEach(([name, value]) =>
    cover.style.setProperty(name, `url(${JSON.stringify(value)})`),
  );
...
function renderProfile(): void {
  ...
  const sceneId = text(
    profileValue(profile, "background_scene_id", "backgroundSceneId"),
  );
  const cover =
    text(profileValue(profile, "profile_cover", "profileCover")) ||
    (sceneId === "default" ? "tide" : sceneId);
  const avatarUrl = text(profileValue(profile, "avatar_url", "avatarUrl"));
  const createdAt = profileValue(profile, "created_at", "createdAt");
  byId("user-profile-cover")?.setAttribute(
    "data-cover",
    PROFILE_COVERS.has(cover) ? cover : "tide",
  );
  void loadProfileBackgroundLayers(sceneId || "default");
  byId("user-value-name")!.textContent = nickname;
  byId("user-profile-bio")!.textContent =
    bio ||
    (language() === "en"
      ? "This player has not added a bio yet."
      : "这个玩家还没有填写简介。");
  byId("user-profile-about")!.textContent =
    bio ||
    (language() === "en"
      ? "This player has not added a bio yet."
      : "这个玩家还没有填写简介。");
...
function statForMode(modeKey: string): JsonRecord {
  return (
    state.stats.byMode.find((item) => text(item.mode_key) === modeKey) || {}
  );
}
...
function renderFeaturedModes(): void {
  ...
  host.replaceChildren(
    ...keys.map((modeKey) => {
      ...
      metrics.append(
        metric(language() === "en" ? "Best score" : "最高分", stats.best_score),
        metric(language() === "en" ? "Best tile" : "最大方块", stats.best_tile),
      );
...
function renderStatsContent(): void {
  byId("user-profile-mode-count")!.textContent = formatNumber(
    state.stats.byMode.length,
  );
  renderPerformance();
  renderRecentRecords();
  renderFeaturedModes();
...
function fastest2048Ms(stats: JsonRecord): number {
  return integer(stats.fastest_2048_ms ?? stats.fastest2048Ms);
}

function renderPerformance(): void {
  const host = byId("user-performance-grid");
  if (!host) return;
  const modes = state.stats.byMode
    .slice()
    .sort(
      (left, right) => integer(right.record_count) - integer(left.record_count),
    );
  if (!modes.length) {
    emptyMessage(
      host,
      language() === "en" ? "No mode results yet." : "暂无模式成绩。",
    );
    return;
  }
  host.replaceChildren(
    ...modes.map((stats) => {
      ...
      const fastest2048 = formatDuration(fastest2048Ms(stats));
      detail.textContent =
        language() === "en"
...
function renderRecentRecords(): void {
  ...
  if (!recentRecords.length) {
    emptyMessage(
      host,
      language() === "en" ? "No game records yet." : "暂无游戏记录。",
    );
    return;
  }
  host.replaceChildren(
    ...recentRecords.map((item) => {
      ...
      score.textContent =
        language() === "en"
          ...
          : `${formatNumber(item.score)} 分`;
      const tile = document.createElement("span");
      tile.textContent =
        language() === "en"
          ? `Tile ${formatNumber(item.best_tile)}`
          : `方块 ${formatNumber(item.best_tile)}`;
      ...
      row.addEventListener("click", () => {
        ...
        byId("user-record-list")?.scrollIntoView({
          block: "nearest",
          behavior: "smooth",
        });
      });
...
function normalizeAchievements(value: unknown): EarnedAchievement[] {
  return records(value)
    .map((item) => {
      const achievement = record(item.achievement) as JsonRecord & {
        id: string;
      };
      ...
      return {
        ...
        showcaseSlot:
          rawSlot >= 1 && rawSlot <= 3 ? (rawSlot as 1 | 2 | 3) : null,
      };
    })
...
async function loadAchievements(userId: number): Promise<void> {
  ...
  const result = await apiClient?.request(
    `/user/${encodeURIComponent(String(userId))}/achievements`,
    { method: "GET" },
  );
...
function renderAchievements(errorMessage = ""): void {
  ...
  if (count)
    count.textContent = errorMessage
      ? "--"
      : formatNumber(earnedAchievements.length);
  ...
  if (wall) {
    if (errorMessage || !earnedAchievements.length) {
      ...
      const sortable = earnedAchievements.map((entry) => ({
        ...entry.achievement,
        entry,
      }));
      const ordered = groupAchievementFamilies(sortable)
      ...
      wall.replaceChildren(
        ...ordered.map((entry) => {
          ...
          earnedAt.textContent =
            language() === "en"
              ? `Earned ${formatDate(entry.earnedAt)}`
              : `获得于 ${formatDate(entry.earnedAt)}`;
  ...
  const bySlot = new Map(
    earnedAchievements
      .filter((item) => item.showcaseSlot)
      .map((item) => [item.showcaseSlot, item]),
  );
  showcase.replaceChildren(
    ...([1, 2, 3] as const).map((slot) => {
      ...
      if (!entry) {
        ...
        empty.textContent =
          language() === "en" ? `Slot ${slot}` : `展示位 ${slot}`;
        return empty;
      }
...
function activateTab(tab: ProfileTab): void {
  document
    .querySelectorAll<HTMLButtonElement>("[data-user-tab]")
    .forEach((button) => {
  ...
  document
    .querySelectorAll<HTMLElement>("[data-user-panel]")
    .forEach((panel) => {
...
function bindProfileCoverMotion(): void {
  ...
  const flush = (): void => {
    ...
    cover.style.setProperty(
      "--profile-cover-glow-x",
      `${(x * 3).toFixed(2)}px`,
    );
    cover.style.setProperty(
      "--profile-cover-glow-y",
      `${(y * 1.8).toFixed(2)}px`,
    );
    cover.style.setProperty(
      "--profile-cover-city-x",
      `${(x * 2.2).toFixed(2)}px`,
    );
    cover.style.setProperty(
      "--profile-cover-city-y",
      `${(y * 0.8).toFixed(2)}px`,
    );
    cover.style.setProperty(
      "--profile-cover-foreground-x",
      `${(x * 4.2).toFixed(2)}px`,
    );
    cover.style.setProperty(
      "--profile-cover-foreground-y",
      `${(y * 1.8).toFixed(2)}px`,
    );
    cover.style.setProperty(
      "--profile-cover-light-x",
      `${(-x * 0.55).toFixed(2)}px`,
    );
    cover.style.setProperty(
      "--profile-cover-light-y",
      `${(-y * 0.45).toFixed(2)}px`,
    );
  };
...
function bioNoticeCopy(): string {
  ...
  const copies: Record<string, [string, string]> = {
    ...
    submitted: [
      "已提交审核。当前公开简介保持不变，审核通过后刷新页面即可看到。",
      "Submitted for review. Your public bio stays unchanged; refresh after approval to see it.",
    ],
    queued: [
      "已提交审核，正在排队中。当前公开简介保持不变。",
      "Submitted for review and queued. Your public bio stays unchanged.",
    ],
    pending: [
      "已有简介正在审核，请等待完成后再提交。",
      "Another bio is being reviewed. Please wait for it to finish.",
    ],
    rate_limited: [
      `提交次数已达上限，可在 ${value} 后重试。`,
      `Submission limit reached. Try again after ${value}.`,
    ],
    blocked: [
      `简介编辑暂时禁用至 ${value}。`,
      `Bio editing is disabled until ${value}.`,
    ],
    conflict: [
      "主页已在其他位置更新，请刷新页面后重试。",
      "The profile changed elsewhere. Refresh the page and try again.",
    ],
    unavailable: [
      "审核服务暂不可用，当前公开简介未更改，请稍后重试。",
      "The review service is unavailable. Your public bio was not changed; try again later.",
    ],
    validation: [
      "简介无法提交，请检查长度后重试。",
      "The bio could not be submitted. Check its length and try again.",
    ],
    unauthorized: [
      "登录状态已失效，请重新登录后再试。",
    ...
    network: [
      "网络请求失败，当前公开简介未更改，请稍后重试。",
      "The network request failed. Your public bio was not changed; try again later.",
    ],
  };
...
function hasActiveModeration(): boolean {
  return moderationHistory.some((item) =>
    ACTIVE_MODERATION_STATUSES.has(text(item.status)),
  );
}
...
function renderModerationHistory(): void {
  ...
  blocked.textContent = isBioBlocked()
    ? language() === "en"
      ? `Bio editing is disabled until ${formatDate(bioBlockedUntil)}.`
      : `简介编辑暂时禁用至 ${formatDate(bioBlockedUntil)}。`
    : "";
  if (moderationHistoryState === "loading") {
    emptyMessage(
      host,
      language() === "en" ? "Loading review history…" : "正在读取审核记录…",
    );
    ...
    host.replaceChildren(
      ...moderationHistory.map((item) => {
        ...
        const safeReason = reasonCopy(
          profileValue(item, "reason_code", "reasonCode"),
        );
        if (safeReason) {
...
async function loadModerationHistory(): Promise<void> {
  ...
  const result = await apiClient.request(
    "/user/me/moderation-submissions?limit=20",
    { method: "GET" },
  );
...
function avatarNoticeCopy(): string {
  ...
  const copies: Record<string, [string, string]> = {
    ...
    too_large: [
      "头像文件不能超过 200 KB。",
      "The avatar file must not exceed 200 KB.",
    ],
    saving: ["正在处理并提交头像…", "Processing and submitting the avatar…"],
    submitted: [
      "头像已提交审核。审核通过前继续显示当前头像。",
      "Avatar submitted for review. Your current avatar remains visible until approval.",
    ],
    pending: [
      "已有头像正在审核，请等待完成。",
      "Another avatar is already under review.",
    ],
    rate_limited: [
      `每 7 天最多上传一次，可在 ${value} 后重试。`,
      `You can upload once every 7 days. Try again after ${value}.`,
    ],
    conflict: [
      "头像状态已变化，请刷新页面后重试。",
      "The avatar state changed. Refresh and try again.",
    ],
    unavailable: [
      "头像审核暂不可用，当前头像未更改。",
      "Avatar review is unavailable. Your current avatar was not changed.",
    ],
    validation: [
      "头像未通过文件校验，请检查格式、尺寸和内容。",
      "The avatar failed file validation. Check its format, dimensions, and content.",
    ],
    unauthorized: [
      "登录状态已失效，请重新登录后再试。",
      "Your session expired. Sign in and try again.",
    ...
    network: [
      "头像上传失败，当前头像未更改，请稍后重试。",
      "Avatar upload failed. Your current avatar was not changed; try again later.",
    ],
  };
...
function avatarIsActive(): boolean {
  return ACTIVE_MODERATION_STATUSES.has(
    text(
      profileValue(
        avatarSubmission || {},
        "moderation_status",
...
function selectedAvatarFile(): File | null {
  return (
    byId<HTMLInputElement>("user-profile-avatar-input")?.files?.[0] || null
  );
}

function avatarFileProblem(
  file: File | null,
): "invalid_type" | "too_large" | "" {
  if (!file) return "";
...
function renderAvatarSubmission(): void {
  ...
  if (avatarSubmissionState === "loading") {
    emptyMessage(
      host,
      language() === "en" ? "Loading avatar review…" : "正在读取头像审核状态…",
    );
  } else if (avatarSubmissionState === "error") {
    emptyMessage(
      host,
      language() === "en"
        ? "Avatar review state could not be loaded."
        : "头像审核状态加载失败。",
    );
    ...
    status.textContent = statusCopy(
      text(
        profileValue(avatarSubmission, "moderation_status", "moderationStatus"),
      ),
    );
    const time = document.createElement("time");
    const submittedAt = text(
      profileValue(avatarSubmission, "submitted_at", "submittedAt"),
    );
    time.dateTime = submittedAt;
    time.textContent = formatDate(submittedAt);
    heading.append(status, time);
    row.appendChild(heading);
    const safeReason = reasonCopy(
      profileValue(avatarSubmission, "reason_code", "reasonCode"),
    );
    if (safeReason) {
      const reason = document.createElement("p");
      reason.textContent = safeReason;
...
async function loadAvatarSubmission(): Promise<void> {
  ...
  if (!state.isOwnProfile) return;
  if (result.success) {
    avatarSubmission = Object.keys(record(result.data)).length
      ? record(result.data)
      : null;
...
async function uploadProfileAvatar(event: SubmitEvent): Promise<void> {
  ...
  if (avatarSaving || !apiClient || !state.isOwnProfile || !editModeEnabled)
    return;
  const file = selectedAvatarFile();
  const problem = avatarFileProblem(file);
  if (!file || problem) {
  ...
  if (!result.success) {
    ...
    if (
      ...
      reasonCode === "content_review_pending"
    ) {
      ...
      avatarNotice = {
        key: "rate_limited",
        value: formatDate(nextAllowedAt),
        kind: "error",
      };
    } else if (
...
function availableFeaturedModes(): JsonRecord[] {
  ...
  return state.stats.byMode.filter(
    (item) =>
      integer(item.record_count) > 0 || selected.has(text(item.mode_key)),
  );
}

function syncFeaturedChoiceLimit(): void {
  const choices = Array.from(
    document.querySelectorAll<HTMLInputElement>("#user-featured-choices input"),
  );
...
function prepareFeaturedDialog(): void {
  ...
  if (modes.length) {
    ...
    emptyMessage(
      host,
      language() === "en"
        ? "Play a mode before featuring it."
        : "游玩并保存记录后，才能选择代表模式。",
    );
  }
...
async function saveFeaturedModes(event: SubmitEvent): Promise<void> {
  ...
  const selected = Array.from(
    document.querySelectorAll<HTMLInputElement>(
      "#user-featured-choices input:checked",
    ),
  ...
  if (!result.success) {
    setDialogStatus(
      "user-featured-save-status",
      text(result.error) || (language() === "en" ? "Save failed" : "保存失败"),
      "error",
    );
    return;
  }
...
async function saveProfileBio(event: SubmitEvent): Promise<void> {
  ...
  bioNotice = {
    key: text(queue.status) === "queued" ? "queued" : "submitted",
    kind: "ok",
  };
  await loadModerationHistory();
  renderBioNotice();
...
function renderBackgroundCatalog(): void {
  ...
  if (!backgroundCatalog.length) {
    host.textContent =
      language() === "en"
        ? "No published backgrounds are available."
        : "暂无已发布背景。";
    return;
  }
  const current =
    text(
      profileValue(state.profile, "background_scene_id", "backgroundSceneId"),
    ) || backgroundCatalogDefaultSceneId;
  host.replaceChildren(
    ...backgroundCatalog.map((scene) => {
...
async function loadBackgroundCatalog(): Promise<void> {
  ...
  if (status)
    status.textContent = language() === "en" ? "Loading…" : "正在加载…";
  ...
  backgroundCatalogDefaultSceneId = result.success
    ? text(catalog.default_scene_id)
    : "";
...
async function saveProfileBackground(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  if (!apiClient || backgroundSaving || !state.isOwnProfile || !editModeEnabled)
    return;
  const selectedSceneId =
    document.querySelector<HTMLInputElement>(
      '#user-profile-background-choices input[name="background_scene_id"]:checked',
    )?.value || "";
  if (!selectedSceneId) return;
  backgroundSaving = true;
  const status = byId("user-profile-background-status");
  if (status)
    status.textContent = language() === "en" ? "Saving…" : "正在保存…";
  const revision = integer(profileValue(state.profile, "revision", "revision"));
  const result = await apiClient.request("/user/me/profile", {
    method: "PATCH",
  ...
  if (status)
    status.textContent =
      language() === "en" ? "Background saved." : "背景已保存。";
}
...
function applyLanguage(): void {
  ...
  const copies: Array<[string, string, string]> = [
    ...
    [
      ".user-profile-sidebar .user-profile-panel:nth-child(1) h2",
      "关于我",
      "About",
    ],
    [
      ".user-profile-sidebar .user-profile-panel:nth-child(2) h2",
      "展示成就",
      "Showcase",
    ],
    [
      ".user-profile-sidebar .user-profile-panel:nth-child(3) h2",
      "主页信息",
      "Profile",
    ],
    [".user-profile-facts div:nth-child(1) dt", "参与模式", "Modes played"],
    [".user-profile-facts div:nth-child(2) dt", "成就数量", "Achievements"],
    ["#user-tab-overview", "概览", "Overview"],
    ["#user-tab-performance", "模式成绩", "Mode results"],
    ["#user-tab-records", "游戏记录", "Game records"],
    ["#user-tab-achievements", "成就", "Achievements"],
    [
      "#user-panel-overview .user-profile-section:nth-child(1) h2",
      "代表模式",
      "Featured modes",
    ],
    [
      "#user-panel-overview .user-profile-section:nth-child(2) h2",
      "最近游戏",
      "Recent games",
    ],
    ["[data-user-jump='records']", "查看全部记录", "View all records"],
    ["#user-panel-performance h2", "按模式汇总的个人成绩", "Results by mode"],
    ["#user-panel-achievements h2", "已获得成就", "Earned achievements"],
    ["[data-user-mode-family='all']", "全部", "All"],
    ["[data-user-mode-family='pow2']", "2 的幂", "Power of two"],
    ["[data-user-mode-family='fibonacci']", "斐波那契", "Fibonacci"],
    ["[data-user-mode-family='diagonal']", "八方向", "8-Direction"],
    ["[data-user-mode-family='special']", "特殊模式", "Special"],
    ["#user-featured-dialog-title", "编辑首页展示", "Edit featured modes"],
    ["#user-profile-avatar-editor-title", "更换头像", "Change avatar"],
    ["#user-profile-avatar-label", "选择头像", "Choose avatar"],
    [
      "#user-profile-avatar-help",
      "支持 JPEG、PNG、WebP，文件不超过 200 KB；每 7 天最多上传一次，审核通过前继续显示当前头像。",
      "JPEG, PNG, or WebP up to 200 KB. You can upload once every 7 days; your current avatar remains until approval.",
    ],
    [
      "#user-profile-avatar-review-title",
      "最新头像审核",
      "Latest avatar review",
    ],
    ["#user-profile-avatar-save", "提交头像审核", "Submit avatar for review"],
    ["#user-profile-bio-editor-title", "编辑主页简介", "Edit profile bio"],
    [".user-profile-bio-label-row label", "主页简介", "Profile bio"],
    [
      "#user-profile-bio-help",
      "简介会发送给 DeepSeek 自动审核；审核通过前继续显示当前公开简介。留空保存表示清空简介。",
      "Your bio is sent to DeepSeek for automated review. The current public bio remains until approval; submit an empty bio to clear it.",
    ],
    ["#user-profile-bio-save", "提交审核", "Submit for review"],
    ["#user-profile-moderation-title", "简介审核记录", "Bio review history"],
    [
      "#user-profile-background-editor-title",
      "编辑主页背景",
      "Edit profile background",
    ],
    [
      "#user-profile-background-help",
      "只显示白天预览；页面会按自动、白天或夜晚状态切换同一场景。",
      "Only day previews are shown. The same scene switches automatically for auto, day, or night mode.",
    ],
...
function bindEnhancedEvents(): void {
  window.addEventListener("userprofilestatechange", (event) =>
    syncLegacyState((event as CustomEvent<unknown>).detail),
  );
  document
    .querySelectorAll<HTMLButtonElement>("[data-user-tab]")
    .forEach((button) => {
      button.addEventListener("click", () =>
        activateTab(button.dataset.userTab as ProfileTab),
      );
    });
  document
    .querySelectorAll<HTMLButtonElement>("[data-user-jump]")
    .forEach((button) => {
      button.addEventListener("click", () =>
        activateTab(button.dataset.userJump as ProfileTab),
      );
    });
  document
    .querySelectorAll<HTMLButtonElement>("[data-user-dialog-close]")
    .forEach((button) => {
      button.addEventListener("click", () =>
        closeDialog(button.dataset.userDialogClose || ""),
      );
    });
  ...
  byId<HTMLFormElement>("user-featured-form")?.addEventListener(
    "submit",
    (event) => void saveFeaturedModes(event),
  );
  byId<HTMLInputElement>("user-profile-avatar-input")?.addEventListener(
    "change",
    previewAvatarInput,
  );
  byId<HTMLFormElement>("user-profile-avatar-form")?.addEventListener(
    "submit",
    (event) => void uploadProfileAvatar(event),
  );
  byId<HTMLTextAreaElement>("user-profile-bio-input")?.addEventListener(
    "input",
    syncBioInputState,
  );
  byId<HTMLFormElement>("user-profile-bio-form")?.addEventListener(
    "submit",
    (event) => void saveProfileBio(event),
  );
  byId<HTMLFormElement>("user-profile-background-form")?.addEventListener(
    "submit",
    (event) => void saveProfileBackground(event),
  );
  window.addEventListener("uilanguagechange", applyLanguage);
  window.addEventListener("storage", (event) => {
    if (event.key === "ui_language_v1") applyLanguage();
  });
  new MutationObserver(() => {
    ...
    attributes: true,
    attributeFilter: ["data-night-background"],
  });
...
export async function bootstrapUserProfilePage(): Promise<void> {
  ...
  const { installUserProfileLegacyRuntime } = await import(
    "../bootstrap/user-profile-legacy-runtime"
  );
