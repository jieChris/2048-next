# Game Manager Runtime Accessor Helpers Retirement Implementation Plan
...
**Goal:** Retire `core_game_manager_runtime_accessor_helpers_runtime.js` from modern active manifests and the Vite home startup bundle while preserving the legacy nomodule compatibility path.

**Architecture:** `src/bootstrap/game-manager-runtime-accessor-helpers-runtime.ts` installs the migrated runtime-accessor helper globals before any active home-family legacy scripts load. The legacy `js/core_game_manager_runtime_accessor_helpers_runtime.js` file stays available for the separate nomodule path, but active manifests and the Vite startup bundle should no longer import or ship it.
...
Expected: FAIL because `core_game_manager_runtime_accessor_helpers_runtime.js` is not yet listed in either retired-reference collection.
...
- Preserve: `js/core_game_manager_runtime_accessor_helpers_runtime.js`
- Preserve: `public/js/legacy_index_nomodule_loader.js`

- [x] **Step 1: Register runtime-accessor helpers as retired in audit rules**
...
Remove `coreGameManagerRuntimeAccessorHelpersRuntimeUrl` import and array entries from:
...
For capped/play/replay manifests without ranked checkpoint, keep:
...
Expected: all PASS. If `audit:game-manager` fails due to a legacy-chain expectation, inspect the failure before changing rules because Stage 1DK does not remove the legacy nomodule chain.
...
- Modify: `docs/superpowers/plans/2026-06-17-game-manager-runtime-accessor-helpers-retirement.md`

- [x] **Step 1: Update architecture guardrails**

Add a top entry saying Stage 1DK retired `core_game_manager_runtime_accessor_helpers_runtime.js` from modern active manifests and the Vite home startup bundle, while preserving the legacy nomodule compatibility reference.

- [x] **Step 2: Update roadmap milestones**

Use a byte-safe top insertion for `docs/ROADMAP_MILESTONES.md` because the file contains non-UTF-8 bytes. Add the same Stage 1DK summary and verification commands.
...
- RED: `npx vitest run tests/unit/entry-manifest-audit-helpers.spec.ts` failed with 2 expected missing-reference assertions for `core_game_manager_runtime_accessor_helpers_runtime.js`.
- GREEN: `npx vitest run tests/unit/entry-manifest-audit-helpers.spec.ts` passed 121 tests.
- `npm run audit:entry-manifest`
- `npm run audit:game-manager`
- `npm run audit:service-boundary`
- `npm run audit:page-legacy-runtime-boundary`
- `npm run build`
- `PW_WEB_PORT=4336 npm run test:smoke:index-ui` passed 9 tests.
...
### Task 5: Ship Stage 1DK

**Files:**
- Commit all Stage 1DK changes.
- Push branch `frontend-runtime-accessor-helpers-stage1dk-retirement`.
- Open PR titled `[codex] retire runtime-accessor game manager helpers runtime`.
...
Expected: only Stage 1DK files changed and no whitespace errors.
...
```bash
git add scripts/entry-manifest-audit.mjs src/entries/home-family-shared.ts src/entries/play-runtime-scripts.ts src/entries/replay-runtime-scripts.ts vite.config.ts tests/unit/entry-manifest-audit-helpers.spec.ts docs/ARCHITECTURE_GUARDRAILS.md docs/ROADMAP_MILESTONES.md docs/superpowers/plans/2026-06-17-game-manager-runtime-accessor-helpers-retirement.md
git commit -m "refactor: retire runtime-accessor game manager helpers runtime"
```
...
```bash
git push -u origin frontend-runtime-accessor-helpers-stage1dk-retirement
gh pr create --title "[codex] retire runtime-accessor game manager helpers runtime" --body-file /tmp/stage1dk-pr-body.md
```
