# Home Guide Finish Host Runtime Boundary Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Retire `js/core_home_guide_finish_host_runtime.js` from active play/home runtime manifests by installing `CoreHomeGuideFinishHostRuntime` from the tested TypeScript bootstrap boundary.

**Architecture:** `src/bootstrap/home-guide-finish-host.ts` already owns the legacy-equivalent finish lifecycle and context delegation behavior. This phase adds the legacy global runtime shape and installs it during home-family bootstrap before legacy scripts load, then blocks the retired script from active manifests through `entry-manifest-audit`.
...
**Files:**
- Modify: `tests/unit/bootstrap-home-guide-finish-host.spec.ts`
- Modify: `tests/unit/entry-manifest-audit-helpers.spec.ts`

- [ ] **Step 1: Add failing installer assertions**

Add imports for `createHomeGuideFinishHostRuntime`, `installHomeGuideFinishHostRuntime`, and `HomeGuideFinishHostRuntime`, then add these tests near the top of `describe("bootstrap home guide finish host", ...)`:
...
Add this test after the home-guide-panel-host retired registry assertion:
...
```bash
npx vitest run tests/unit/bootstrap-home-guide-finish-host.spec.ts
...
### Task 2: Install `CoreHomeGuideFinishHostRuntime` from TypeScript

**Files:**
- Modify: `src/bootstrap/home-guide-finish-host.ts`
- Modify: `src/entries/home-family-bootstrap.ts`

- [ ] **Step 1: Add runtime interfaces and installer**

Add the legacy runtime shape to `src/bootstrap/home-guide-finish-host.ts`:
...
Add the installer after `applyHomeGuideFinishFromContext`:
...
Expected: FAIL while `core_home_guide_finish_host_runtime.js` remains in `src/entries/play-runtime-scripts.ts` and `src/entries/home-family-shared.ts`.

- [ ] **Step 4: Remove active manifest references**

Remove `coreHomeGuideFinishHostRuntimeUrl` imports and array entries from:
...
Do not delete `js/core_home_guide_finish_host_runtime.js`.
...
Prepend a Stage-1AR section documenting:
- `CoreHomeGuideFinishHostRuntime` is installed from `src/bootstrap/home-guide-finish-host.ts`.
- `src/bootstrap/home-guide-finish-host.ts` owns home-guide finish lifecycle and context delegation.
- Legacy runtime global shape is preserved.
- `js/core_home_guide_finish_host_runtime.js` is retired from active manifests without deletion.
- `entry-manifest-audit` blocks the retired script and symbol.

- [ ] **Step 2: Prepend roadmap milestone**

Prepend a Stage-1AR section with `WS-runtime-41`, `status: done`, and evidence commands.
...
```bash
npx vitest run tests/unit/bootstrap-home-guide-finish-host.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts
```
...
```bash
git add src/bootstrap/home-guide-finish-host.ts src/entries/home-family-bootstrap.ts src/entries/play-runtime-scripts.ts src/entries/home-family-shared.ts scripts/entry-manifest-audit.mjs tests/unit/bootstrap-home-guide-finish-host.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts docs/ARCHITECTURE_GUARDRAILS.md docs/ROADMAP_MILESTONES.md docs/superpowers/plans/2026-06-13-home-guide-finish-host-runtime-boundary.md
git commit -m "refactor: install home guide finish host runtime from ts"
```

- [ ] **Step 5: Push, open PR, wait for CI, merge**

Push branch `frontend-runtime-ts-boundary-stage1ar-home-guide-finish-host`, open a draft PR, mark ready after local gates are captured, wait for GitHub checks, then merge only after CI is green.
...
- Type consistency: Runtime names use `HomeGuideFinishHostRuntime`, `CoreHomeGuideFinishHostRuntime`, and `installHomeGuideFinishHostRuntime` consistently across tests, implementation, bootstrap, and docs.
