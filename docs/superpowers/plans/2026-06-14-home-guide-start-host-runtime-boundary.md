# Home Guide Start Host Runtime Boundary Implementation Plan
...
**Goal:** Retire `js/core_home_guide_start_host_runtime.js` from active play/home runtime manifests by installing `CoreHomeGuideStartHostRuntime` from the tested TypeScript bootstrap boundary.

**Architecture:** `src/bootstrap/home-guide-start-host.ts` already owns the legacy-equivalent home-guide start lifecycle behavior. This phase adds the legacy global runtime shape and installs it during home-family bootstrap before legacy scripts load, then blocks the retired script from active manifests through `entry-manifest-audit`.
...
**Files:**
- Modify: `tests/unit/bootstrap-home-guide-start-host.spec.ts`
- Modify: `tests/unit/entry-manifest-audit-helpers.spec.ts`

- [ ] **Step 1: Add failing installer assertions**

Add imports for `createHomeGuideStartHostRuntime`, `installHomeGuideStartHostRuntime`, and `HomeGuideStartHostRuntime`, then add these tests near the top of `describe("bootstrap home guide start host", ...)`:
...
Add this test after the home-guide-finish-host retired registry assertion:
...
```bash
npx vitest run tests/unit/bootstrap-home-guide-start-host.spec.ts
...
### Task 2: Install `CoreHomeGuideStartHostRuntime` from TypeScript

**Files:**
- Modify: `src/bootstrap/home-guide-start-host.ts`
- Modify: `src/entries/home-family-bootstrap.ts`

- [ ] **Step 1: Add runtime interfaces and installer**

Add the legacy runtime shape to `src/bootstrap/home-guide-start-host.ts`:
...
Add the installer after `applyHomeGuideStart`:
...
Expected: FAIL while `core_home_guide_start_host_runtime.js` remains in `src/entries/play-runtime-scripts.ts` and `src/entries/home-family-shared.ts`.

- [ ] **Step 4: Remove active manifest references**

Remove `coreHomeGuideStartHostRuntimeUrl` imports and array entries from:
...
Do not delete `js/core_home_guide_start_host_runtime.js`.
...
Prepend a Stage-1AS section documenting:
- `CoreHomeGuideStartHostRuntime` is installed from `src/bootstrap/home-guide-start-host.ts`.
- `src/bootstrap/home-guide-start-host.ts` owns home-guide start lifecycle orchestration.
- Legacy runtime global shape is preserved.
- `js/core_home_guide_start_host_runtime.js` is retired from active manifests without deletion.
- `entry-manifest-audit` blocks the retired script and symbol.

- [ ] **Step 2: Prepend roadmap milestone**

Prepend a Stage-1AS section with `WS-runtime-42`, `status: done`, and evidence commands.
...
```bash
npx vitest run tests/unit/bootstrap-home-guide-start-host.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts
```
...
```bash
git add src/bootstrap/home-guide-start-host.ts src/entries/home-family-bootstrap.ts src/entries/play-runtime-scripts.ts src/entries/home-family-shared.ts scripts/entry-manifest-audit.mjs tests/unit/bootstrap-home-guide-start-host.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts docs/ARCHITECTURE_GUARDRAILS.md docs/ROADMAP_MILESTONES.md docs/superpowers/plans/2026-06-14-home-guide-start-host-runtime-boundary.md
git commit -m "refactor: install home guide start host runtime from ts"
```

- [ ] **Step 5: Push, open PR, wait for CI, merge**

Push branch `frontend-runtime-ts-boundary-stage1as-home-guide-start-host`, open a draft PR, mark ready after local gates are captured, wait for GitHub checks, then merge only after CI is green.
...
- Type consistency: Runtime names use `HomeGuideStartHostRuntime`, `CoreHomeGuideStartHostRuntime`, and `installHomeGuideStartHostRuntime` consistently across tests, implementation, bootstrap, and docs.
