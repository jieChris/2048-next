# Home Guide Startup Host Runtime Boundary Implementation Plan
...
**Goal:** Retire `js/core_home_guide_startup_host_runtime.js` from active play/home runtime manifests by installing `CoreHomeGuideStartupHostRuntime` from the tested TypeScript bootstrap boundary.

**Architecture:** `src/bootstrap/home-guide-startup-host.ts` already owns the legacy-equivalent auto-start decision delegation, delay resolution, and scheduled guide start behavior. This phase adds the legacy global runtime shape and installs it during home-family bootstrap before legacy scripts load, then blocks the retired script from active manifests through `entry-manifest-audit`.
...
**Files:**
- Modify: `tests/unit/bootstrap-home-guide-startup-host.spec.ts`
- Modify: `tests/unit/entry-manifest-audit-helpers.spec.ts`

- [ ] **Step 1: Add failing installer assertions**

Add imports for `createHomeGuideStartupHostRuntime`, `installHomeGuideStartupHostRuntime`, and `HomeGuideStartupHostRuntime`, then add these tests near the top of `describe("bootstrap home guide startup host", ...)`:
...
Add this test after the home-guide-settings-host retired registry assertion:
...
```bash
npx vitest run tests/unit/bootstrap-home-guide-startup-host.spec.ts
...
### Task 2: Install `CoreHomeGuideStartupHostRuntime` from TypeScript

**Files:**
- Modify: `src/bootstrap/home-guide-startup-host.ts`
- Modify: `src/entries/home-family-bootstrap.ts`

- [ ] **Step 1: Add runtime interfaces and installer**

Add the legacy runtime shape to `src/bootstrap/home-guide-startup-host.ts`:
...
Add the installer after `applyHomeGuideAutoStart`:
...
Expected: FAIL while `core_home_guide_startup_host_runtime.js` remains in `src/entries/play-runtime-scripts.ts` and `src/entries/home-family-shared.ts`.

- [ ] **Step 4: Remove active manifest references**

Remove `coreHomeGuideStartupHostRuntimeUrl` imports and array entries from:
...
Do not delete `js/core_home_guide_startup_host_runtime.js`.
...
Prepend a Stage-1AX section documenting:
- `CoreHomeGuideStartupHostRuntime` is installed from `src/bootstrap/home-guide-startup-host.ts`.
- `src/bootstrap/home-guide-startup-host.ts` owns auto-start state delegation and scheduled start orchestration.
- Legacy runtime global shape is preserved.
- `js/core_home_guide_startup_host_runtime.js` is retired from active manifests without deletion.
- `entry-manifest-audit` blocks the retired script and symbol.

- [ ] **Step 2: Prepend roadmap milestone**

Prepend a Stage-1AX section with `WS-runtime-47`, `status: done`, and evidence commands.
...
```bash
npx vitest run tests/unit/bootstrap-home-guide-startup-host.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts
```
...
```bash
git add src/bootstrap/home-guide-startup-host.ts src/entries/home-family-bootstrap.ts src/entries/play-runtime-scripts.ts src/entries/home-family-shared.ts scripts/entry-manifest-audit.mjs tests/unit/bootstrap-home-guide-startup-host.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts docs/ARCHITECTURE_GUARDRAILS.md docs/ROADMAP_MILESTONES.md docs/superpowers/plans/2026-06-14-home-guide-startup-host-runtime-boundary.md
git commit -m "refactor: install home guide startup host runtime from ts"
```

- [ ] **Step 5: Push, open PR, wait for CI, merge**

Push branch `frontend-runtime-ts-boundary-stage1ax-home-guide-startup-host`, open a draft PR, mark ready after local gates are captured, wait for GitHub checks, then merge only after CI is green.
...
- Type consistency: Runtime names use `HomeGuideStartupHostRuntime`, `CoreHomeGuideStartupHostRuntime`, and `installHomeGuideStartupHostRuntime` consistently across tests, implementation, bootstrap, and docs.
