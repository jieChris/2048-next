# Home Guide Step Host Runtime Boundary Implementation Plan
...
**Goal:** Retire `js/core_home_guide_step_host_runtime.js` from active play/home runtime manifests by installing `CoreHomeGuideStepHostRuntime` from the tested TypeScript bootstrap boundary.

**Architecture:** `src/bootstrap/home-guide-step-host.ts` already owns the legacy-equivalent step-flow delegation, step-view delegation, and repeated advance orchestration. This phase adds the legacy global runtime shape and installs it during home-family bootstrap before legacy scripts load, then blocks the retired script from active manifests through `entry-manifest-audit`.
...
**Files:**
- Modify: `tests/unit/bootstrap-home-guide-step-host.spec.ts`
- Modify: `tests/unit/entry-manifest-audit-helpers.spec.ts`

- [ ] **Step 1: Add failing installer assertions**

Add imports for `createHomeGuideStepHostRuntime`, `installHomeGuideStepHostRuntime`, and `HomeGuideStepHostRuntime`, then add these tests near the top of `describe("bootstrap home guide step host", ...)`:
...
Add this test after the home-guide-step-flow-host retired registry assertion:
...
```bash
npx vitest run tests/unit/bootstrap-home-guide-step-host.spec.ts
...
### Task 2: Install `CoreHomeGuideStepHostRuntime` from TypeScript

**Files:**
- Modify: `src/bootstrap/home-guide-step-host.ts`
- Modify: `src/entries/home-family-bootstrap.ts`

- [ ] **Step 1: Add runtime interfaces and installer**

Add the legacy runtime shape to `src/bootstrap/home-guide-step-host.ts`:
...
Add the installer after `applyHomeGuideStepOrchestration`:
...
export function installHomeGuideStepHostRuntime(
  ...
  const windowLike =
    options.windowLike ||
    (typeof window === "undefined" ? null : (window as unknown as HomeGuideStepHostRuntimeWindowLike));
...
Expected: FAIL while `core_home_guide_step_host_runtime.js` remains in `src/entries/play-runtime-scripts.ts` and `src/entries/home-family-shared.ts`.

- [ ] **Step 4: Remove active manifest references**

Remove `coreHomeGuideStepHostRuntimeUrl` imports and array entries from:
...
Do not delete `js/core_home_guide_step_host_runtime.js`.
...
Prepend a Stage-1AV section documenting:
- `CoreHomeGuideStepHostRuntime` is installed from `src/bootstrap/home-guide-step-host.ts`.
- `src/bootstrap/home-guide-step-host.ts` owns step-flow delegation, step-view delegation, and repeated advance orchestration.
- Legacy runtime global shape is preserved.
- `js/core_home_guide_step_host_runtime.js` is retired from active manifests without deletion.
- `entry-manifest-audit` blocks the retired script and symbol.

- [ ] **Step 2: Prepend roadmap milestone**

Prepend a Stage-1AV section with `WS-runtime-45`, `status: done`, and evidence commands.
...
```bash
npx vitest run tests/unit/bootstrap-home-guide-step-host.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts
```
...
```bash
git add src/bootstrap/home-guide-step-host.ts src/entries/home-family-bootstrap.ts src/entries/play-runtime-scripts.ts src/entries/home-family-shared.ts scripts/entry-manifest-audit.mjs tests/unit/bootstrap-home-guide-step-host.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts docs/ARCHITECTURE_GUARDRAILS.md docs/ROADMAP_MILESTONES.md docs/superpowers/plans/2026-06-14-home-guide-step-host-runtime-boundary.md
git commit -m "refactor: install home guide step host runtime from ts"
```

- [ ] **Step 5: Push, open PR, wait for CI, merge**

Push branch `frontend-runtime-ts-boundary-stage1av-home-guide-step-host`, open a draft PR, mark ready after local gates are captured, wait for GitHub checks, then merge only after CI is green.
...
- Type consistency: Runtime names use `HomeGuideStepHostRuntime`, `CoreHomeGuideStepHostRuntime`, and `installHomeGuideStepHostRuntime` consistently across tests, implementation, bootstrap, and docs.
