# Guardrail Delta (2026-06-17, Stage-1DK Core Game Manager Runtime Accessor Helpers Retirement)

## Batch Impact
- `core_game_manager_runtime_accessor_helpers_runtime.js` is no longer referenced by active home/capped/play/replay runtime manifests.
- `core_game_manager_runtime_accessor_helpers_runtime.js` is no longer included in `HOME_STANDARD_STARTUP_FILES`, so the Vite-generated home startup bundle no longer carries the legacy runtime-accessor helper runtime.
- `src/bootstrap/game-manager-runtime-accessor-helpers-runtime.ts` remains the modern installer for migrated runtime-accessor helper globals before active legacy scripts load.
- `js/core_game_manager_runtime_accessor_helpers_runtime.js` remains in the repository, and `public/js/legacy_index_nomodule_loader.js` still references it for the separate legacy-browser policy path.
- `entry-manifest-audit` now blocks `core_game_manager_runtime_accessor_helpers_runtime.js` / `coreGameManagerRuntimeAccessorHelpersRuntimeUrl` from re-entering active manifests and blocks the script from re-entering the Vite home startup bundle.
...
# Guardrail Delta (2026-06-17, Stage-1DJ Core Game Manager Runtime Call Helpers Retirement)

## Batch Impact
- `core_game_manager_runtime_call_helpers_runtime.js` is no longer referenced by active home/capped/play/replay runtime manifests.
- `core_game_manager_runtime_call_helpers_runtime.js` is no longer included in `HOME_STANDARD_STARTUP_FILES`, so the Vite-generated home startup bundle no longer carries the legacy runtime-call helper runtime.
- `src/bootstrap/game-manager-runtime-call-helpers-runtime.ts` remains the modern installer for migrated runtime-call helper globals before active legacy scripts load.
- `js/core_game_manager_runtime_call_helpers_runtime.js` remains in the repository, and `public/js/legacy_index_nomodule_loader.js` still references it for the separate legacy-browser policy path.
- `entry-manifest-audit` now blocks `core_game_manager_runtime_call_helpers_runtime.js` / `coreGameManagerRuntimeCallHelpersRuntimeUrl` from re-entering active manifests and blocks the script from re-entering the Vite home startup bundle.
...
- `npm run audit:page-legacy-runtime-boundary`
- `npm run build`
- `PW_WEB_PORT=4335 npm run test:smoke:index-ui`
- `npx playwright test --config=playwright.config.ts --workers=1 tests/smoke/pages-runtime-contract.smoke.spec.ts`
- `npm run verify:prepush`

# Guardrail Delta (2026-06-17, Stage-1DI Core Game Manager Env Helpers Retirement)

## Batch Impact
- `core_game_manager_env_helpers_runtime.js` is no longer referenced by active home/capped/play/replay runtime manifests.
- `core_game_manager_env_helpers_runtime.js` is no longer included in `HOME_STANDARD_STARTUP_FILES`, so the Vite-generated home startup bundle no longer carries the legacy env-helper runtime.
- `src/bootstrap/game-manager-env-helpers-runtime.ts` remains the modern installer for migrated env-helper globals before active legacy scripts load.
- `js/core_game_manager_env_helpers_runtime.js` remains in the repository, and `public/js/legacy_index_nomodule_loader.js` still references it for the separate legacy-browser policy path.
- `entry-manifest-audit` now blocks `core_game_manager_env_helpers_runtime.js` / `coreGameManagerEnvHelpersRuntimeUrl` from re-entering active manifests and blocks the script from re-entering the Vite home startup bundle.
...
## Batch Impact
- `src/core/game-manager-runtime-accessor-helpers.ts` now owns runtime accessor object detection and `GameManager.prototype` getter/resolver/caller registration.
- `src/bootstrap/game-manager-runtime-accessor-helpers-runtime.ts` installs the migrated legacy global helper names before active home-family game-manager scripts load.
- `src/entries/home-family-bootstrap.ts` wires the runtime-accessor helper installer immediately after `installGameManagerRuntimeCallHelpersRuntime()`.
- Active manifest references, `HOME_STANDARD_STARTUP_FILES`, `js/core_game_manager_runtime_accessor_helpers_runtime.js`, and `public/js/legacy_index_nomodule_loader.js` remain intentionally in place; retirement is a later stage.

## Verification
- RED: `npx vitest run tests/unit/core-game-manager-runtime-accessor-helpers.spec.ts`
- RED: `npx vitest run tests/unit/bootstrap-game-manager-runtime-accessor-helpers-runtime.spec.ts`
...
# Guardrail Delta (2026-06-17, Stage-1DF Core Game Manager Runtime Call Helpers TypeScript Boundary)

## Batch Impact
- `src/core/game-manager-runtime-call-helpers.ts` now owns runtime method dispatch, core payload/args resolution, storage runtime calls, score/replay/undo state mutation, and grid cell writes.
- `src/bootstrap/game-manager-runtime-call-helpers-runtime.ts` installs the migrated legacy global helper names before active home-family game-manager scripts load.
- `src/entries/home-family-bootstrap.ts` wires the runtime-call helper installer immediately after `installGameManagerEnvHelpersRuntime()`.
- Active manifest references, `HOME_STANDARD_STARTUP_FILES`, `js/core_game_manager_runtime_call_helpers_runtime.js`, and `public/js/legacy_index_nomodule_loader.js` remain intentionally in place; retirement is a later stage.

## Verification
- RED: `npx vitest run tests/unit/core-game-manager-runtime-call-helpers.spec.ts`
- RED: `npx vitest run tests/unit/bootstrap-game-manager-runtime-call-helpers-runtime.spec.ts`
- GREEN: `npx vitest run tests/unit/bootstrap-game-manager-runtime-call-helpers-runtime.spec.ts tests/unit/core-game-manager-runtime-call-helpers.spec.ts`
- `npm run audit:entry-manifest`
- `npm run audit:game-manager`
- `npm run audit:service-boundary`
- `npm run audit:page-legacy-runtime-boundary`
...
## Batch Impact
- `src/core/game-manager-base-helpers.ts` now owns `createCoreModeDefaultsPayload` and `createCoreModeContextPayload` in addition to the previously migrated low-level call/resolve helpers.
- `src/bootstrap/game-manager-base-helpers-runtime.ts` installs those legacy global helper names before home/play/replay/capped legacy scripts load.
- Active manifest references and the Vite home startup bundle reference to `js/core_game_manager_base_helpers_runtime.js` remain intentionally in place because secondary timer helpers still require a later TypeScript boundary stage.
- The legacy `js/core_game_manager_base_helpers_runtime.js` file remains in place.

## Verification
- RED: `npx vitest run tests/unit/core-game-manager-base-helpers.spec.ts tests/unit/bootstrap-game-manager-base-helpers-runtime.spec.ts`
- GREEN: `npx vitest run tests/unit/bootstrap-game-manager-base-helpers-runtime.spec.ts tests/unit/core-game-manager-base-helpers.spec.ts tests/unit/core-game-manager-base-helpers-runtime.spec.ts tests/unit/entry-manifest-audit-helpers.spec.ts`
- `npm run audit:entry-manifest`
- `npm run audit:game-manager`
- `npm run audit:service-boundary`
...
- `js/core_game_manager_client_record_id_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
...
- `entry-manifest-audit` now blocks `core_game_manager_client_record_id_runtime.js` from re-entering the focused Vite bundled retired registry.
...
## Batch Impact
- `CoreGameSettingsStorageRuntime` remains installed from `src/bootstrap/game-settings-storage-runtime.ts` before home/play/replay/capped legacy scripts load.
...
- `js/core_game_settings_storage_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_game_settings_storage_runtime.js` file remains in place.
- `src/bootstrap/user-profile-legacy-runtime.ts` still imports `../../js/core_game_settings_storage_runtime.js`; legacy user-profile compatibility remains separate from the active bundle retirement.
...
- `entry-manifest-audit` now blocks `core_game_settings_storage_runtime.js` from re-entering the focused Vite bundled retired registry.
...
## Batch Impact
- `CoreMoveApplyRuntime` remains installed from `src/bootstrap/move-apply-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_move_apply_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_move_apply_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_move_apply_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_move_apply_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-16, Stage-1CS Replay-Loop Bundle Runtime Retirement)

## Batch Impact
- `CoreReplayLoopRuntime` remains installed from `src/bootstrap/replay-loop-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_loop_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_loop_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_loop_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_loop_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-16, Stage-1CR Replay-Control Bundle Runtime Retirement)

## Batch Impact
- `CoreReplayControlRuntime` remains installed from `src/bootstrap/replay-control-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_control_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_control_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_control_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_control_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-16, Stage-1CQ Replay-Flow Bundle Runtime Retirement)

## Batch Impact
- `CoreReplayFlowRuntime` remains installed from `src/bootstrap/replay-flow-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_flow_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_flow_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_flow_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_flow_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-16, Stage-1CP Replay-Timer Bundle Runtime Retirement)

## Batch Impact
- `CoreReplayTimerRuntime` remains installed from `src/bootstrap/replay-timer-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_timer_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_timer_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_timer_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_timer_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-16, Stage-1CO Replay-Lifecycle Bundle Runtime Retirement)

## Batch Impact
- `CoreReplayLifecycleRuntime` remains installed from `src/bootstrap/replay-lifecycle-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_lifecycle_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_lifecycle_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_lifecycle_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_lifecycle_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-16, Stage-1CN Replay-Dispatch Bundle Runtime Retirement)

## Batch Impact
- `CoreReplayDispatchRuntime` remains installed from `src/bootstrap/replay-dispatch-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_dispatch_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_dispatch_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_dispatch_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_dispatch_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CM Replay-Execution Bundle Runtime Retirement)

## Batch Impact
- `CoreReplayExecutionRuntime` remains installed from `src/bootstrap/replay-execution-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_execution_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_execution_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_execution_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_execution_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CL Replay-Import Bundle Runtime Retirement)

## Batch Impact
- `CoreReplayImportRuntime` remains installed from `src/bootstrap/replay-import-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_import_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_import_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_import_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_import_runtime.js` from re-entering the focused Vite bundled retired registry.
...
## Batch Impact
- `CoreReplayCodecRuntime` remains installed from `src/bootstrap/replay-codec-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_replay_codec_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_replay_codec_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_replay_codec_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_replay_codec_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CI Undo-Stack-Entry Bundle Runtime Retirement)

## Batch Impact
- `CoreUndoStackEntryRuntime` remains installed from `src/bootstrap/undo-stack-entry-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_undo_stack_entry_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_undo_stack_entry_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_undo_stack_entry_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_undo_stack_entry_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CH Undo-Restore-Payload Bundle Runtime Retirement)

## Batch Impact
- `CoreUndoRestorePayloadRuntime` remains installed from `src/bootstrap/undo-restore-payload-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_undo_restore_payload_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_undo_restore_payload_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_undo_restore_payload_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_undo_restore_payload_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CG Undo-Tile-Restore Bundle Runtime Retirement)

## Batch Impact
- `CoreUndoTileRestoreRuntime` remains installed from `src/bootstrap/undo-tile-restore-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_undo_tile_restore_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_undo_tile_restore_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_undo_tile_restore_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_undo_tile_restore_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CF Undo-Tile-Snapshot Bundle Runtime Retirement)

## Batch Impact
- `CoreUndoTileSnapshotRuntime` remains installed from `src/bootstrap/undo-tile-snapshot-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_undo_tile_snapshot_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_undo_tile_snapshot_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_undo_tile_snapshot_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_undo_tile_snapshot_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CE Undo-Snapshot Bundle Runtime Retirement)

## Batch Impact
- `CoreUndoSnapshotRuntime` remains installed from `src/bootstrap/undo-snapshot-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_undo_snapshot_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_undo_snapshot_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_undo_snapshot_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_undo_snapshot_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CD Undo-Restore Bundle Runtime Retirement)

## Batch Impact
- `CoreUndoRestoreRuntime` remains installed from `src/bootstrap/undo-restore-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_undo_restore_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_undo_restore_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_undo_restore_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_undo_restore_runtime.js` from re-entering the focused Vite bundled retired registry.
...
# Guardrail Delta (2026-06-15, Stage-1CC Post-Undo-Record Bundle Runtime Retirement)

## Batch Impact
- `CorePostUndoRecordRuntime` remains installed from `src/bootstrap/post-undo-record-runtime.ts` before home/play/replay/capped legacy scripts load.
- `js/core_post_undo_record_runtime.js` was removed from `HOME_STANDARD_STARTUP_FILES` in `vite.config.ts`, preventing the Vite-generated home startup bundle from overwriting the TypeScript-installed runtime.
- The legacy `js/core_post_undo_record_runtime.js` file remains in place.
- `public/js/legacy_index_nomodule_loader.js` still references `core_post_undo_record_runtime.js`; legacy-browser policy remains a separate follow-up stage.
- `entry-manifest-audit` now blocks `core_post_undo_record_runtime.js` from re-entering the focused Vite bundled retired registry.
