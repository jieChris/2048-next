...
- `WS-runtime-111`
  - status: done
  - progress: `core_game_manager_runtime_call_helpers_runtime.js` is retired from active home/capped/play/replay manifests and from `HOME_STANDARD_STARTUP_FILES`; `entry-manifest-audit` now prevents re-entry in both modern paths.
  - follow-up: `src/bootstrap/game-manager-runtime-call-helpers-runtime.ts` remains the active TypeScript installer; `js/core_game_manager_runtime_call_helpers_runtime.js` and `public/js/legacy_index_nomodule_loader.js` remain for the separate legacy-browser policy stage.

## Evidence
- RED: `npx vitest run tests/unit/entry-manifest-audit-helpers.spec.ts` failed before `RETIRED_RUNTIME_SCRIPT_MANIFEST_REFS` and `BUNDLED_RETIRED_RUNTIME_SCRIPT_REFS` included `core_game_manager_runtime_call_helpers_runtime.js`.
- GREEN: `npx vitest run tests/unit/entry-manifest-audit-helpers.spec.ts`
- `npm run audit:entry-manifest`
- `npm run audit:game-manager`
- `npm run audit:service-boundary`
...
# Stage-1DI Core Game Manager Env Helpers Retirement (2026-06-17)

## Phase Decision
- `WS-runtime-110`
  - status: done
  - progress: `core_game_manager_env_helpers_runtime.js` is retired from active home/capped/play/replay manifests and from `HOME_STANDARD_STARTUP_FILES`; `entry-manifest-audit` now prevents re-entry in both modern paths.
  - follow-up: `src/bootstrap/game-manager-env-helpers-runtime.ts` remains the active TypeScript installer; `js/core_game_manager_env_helpers_runtime.js` and `public/js/legacy_index_nomodule_loader.js` remain for the separate legacy-browser policy stage.

## Evidence
- RED: `npx vitest run tests/unit/entry-manifest-audit-helpers.spec.ts` failed before `RETIRED_RUNTIME_SCRIPT_MANIFEST_REFS` and `BUNDLED_RETIRED_RUNTIME_SCRIPT_REFS` included `core_game_manager_env_helpers_runtime.js`.
- GREEN: `npx vitest run tests/unit/entry-manifest-audit-helpers.spec.ts`
- `npm run audit:entry-manifest`
- `npm run audit:game-manager`
- `npm run audit:service-boundary`
...
- `WS-runtime-107`
  - status: done
  - progress: `src/core/game-manager-runtime-call-helpers.ts` now owns runtime method dispatch, core payload/args resolution, storage runtime calls, score/replay/undo state mutation, and grid cell writes; `src/bootstrap/game-manager-runtime-call-helpers-runtime.ts` installs their legacy global names before active home-family game-manager scripts load.
  - follow-up: active manifest references, `HOME_STANDARD_STARTUP_FILES`, `js/core_game_manager_runtime_call_helpers_runtime.js`, and `public/js/legacy_index_nomodule_loader.js` remain intentionally in place for later retirement stages.

## Evidence
- RED: `npx vitest run tests/unit/core-game-manager-runtime-call-helpers.spec.ts` failed while `src/core/game-manager-runtime-call-helpers.ts` exported no helpers.
- RED: `npx vitest run tests/unit/bootstrap-game-manager-runtime-call-helpers-runtime.spec.ts` failed while `src/bootstrap/game-manager-runtime-call-helpers-runtime.ts` did not exist.
...
# Stage-1DE Core Game Manager Env Helpers TypeScript Boundary (2026-06-17)

## Phase Decision
- `WS-runtime-106`
  - status: done
  - progress: `src/core/game-manager-env-helpers.ts` now owns the game-manager environment helpers, and `src/bootstrap/game-manager-env-helpers-runtime.ts` installs their legacy global names before active home-family game-manager scripts load.
  - follow-up: active manifest references, `HOME_STANDARD_STARTUP_FILES`, `js/core_game_manager_env_helpers_runtime.js`, and `public/js/legacy_index_nomodule_loader.js` remain intentionally in place for later retirement stages.

## Evidence
- RED: `npx vitest run tests/unit/core-game-manager-env-helpers.spec.ts` failed while `src/core/game-manager-env-helpers.ts` exported no helpers.
- RED: `npx vitest run tests/unit/bootstrap-game-manager-env-helpers-runtime.spec.ts` failed while `src/bootstrap/game-manager-env-helpers-runtime.ts` did not exist.
...
- `WS-runtime-103`
  - status: done
  - progress: `src/core/game-manager-base-helpers.ts` now owns secondary timer normalization, hierarchy, expanded parent state, descriptor, placement diagnostics, stamping, invalidation, and row-state restore helpers; `src/bootstrap/game-manager-base-helpers-runtime.ts` installs those legacy global names before home/play/replay/capped scripts load.
...
## Evidence
- RED: `npx vitest run tests/unit/core-game-manager-base-helpers.spec.ts` failed before secondary timer helpers were exported.
- RED: `npx vitest run tests/unit/bootstrap-game-manager-base-helpers-runtime.spec.ts` failed before the runtime installer exposed secondary timer helper names.
