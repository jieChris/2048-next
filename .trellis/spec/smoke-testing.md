...
- 账号色板首次接入同步时，页面可能在第一次云端读取前把旧 Web 色板镜像标记为 `dirty + revision 0`；若云端已存在更高 revision，不得直接发起注定冲突的 CAS。等价或本地空状态应采用云端，云端空状态应先基于其 revision 重放本地文档；并发首次写入返回等价结果时应视为同步成功。`source`、`locked` 和本地时间戳属于设备私有元数据，不得把仅有这些差异的色板误报为冲突；颜色、名称、边框、发光等权威视觉字段不同才保留冲突。真实分歧才进入冲突状态，Smoke/单测应分别覆盖这些分支。
