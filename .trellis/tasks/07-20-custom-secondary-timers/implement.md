...
10. 将动态子行图例从完整表达式改为末级数值，复用现有深度缩进，并用页面 Smoke 锁定一级、二级层级与完整表达式标题。
11. 在设置页输入框右侧增加已保存规则预览，复用规则解析与计时器视觉类，支持按母计时器展开并在窄屏下纵向排列。
12. 校正 Fibonacci 合成容量：3x3 母计时器显示到 `4181`，4x4 显示到理论最大值前一项，并同步规则层单测与页面 Smoke。
13. 让练习板摆盘预选块复用 Fibonacci 母计时器终点，并用页面 Smoke 覆盖 4x2、3x3、4x3、4x4 四种棋盘。
