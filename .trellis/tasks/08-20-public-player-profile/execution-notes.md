...
- 2026-08-24：前端提交 `c036ed0c` 的 Actions `32729056398` 在无关的 pages Smoke 暂时失败，发布门禁因此未进入 build/deploy。CI trace 证明网络恢复事件发生时统一刷新 callback 尚在运行，旧调度器把立即唤醒降为 5 秒普通轮询，超过用例 4 秒的能力等待。按最保守路线暂停部署、保留有效断言与超时，先以运行时单测复现 RED，再仅修复调度器的运行中唤醒排队；未回退个人主页修复、未新建分支、未触碰线上前端。
