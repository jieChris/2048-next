...
视觉回归使用独立 `playwright.visual.config.ts`，测试只放在 `tests/visual/`。账号设置游客页保留独立语义断言；全量用例以数据数组登记另外 33 个页面和 18 个弹窗，复用同一套视口、主题、状态安装、API fixture、就绪等待与横向溢出断言。Playwright 原生截图断言负责比较 PNG，普通命令只比较，更新命令显式传入 `--update-snapshots`。
...
- `tests/visual/all-pages.visual.spec.ts`：其余 33 个页面和 18 个弹窗的 408 个视觉场景。
