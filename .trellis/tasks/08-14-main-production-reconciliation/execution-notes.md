...
- 2026-08-15：生产验证确认 PR #223 将独立站点容器的 API upstream 从 `2048-game-api:3001` 回退为容器自身的 `127.0.0.1:3010`，导致 `/health` 返回 502、Admin 门禁返回 500/503；同时该 PR 相对旧 `main` 改写 187 个文件。直接整库回退会重新引入回放计时和新局出数等已修复问题，因此采用最保守恢复路线：先仅恢复既有提交 `e9462d5f` 的容器网络/upstream 契约及必要健康探针，再单独重建主线，不在生产故障期间批量选择文件树。
- 2026-08-15：Trellis `before-dev` 要求的 `.trellis/scripts/get_context.py` 与 `.trellis/spec/guides/index.md` 均不存在，本任务目录也只有旧执行记录、没有 PRD/设计/实施文件。无法按完整发现流程继续，保守降级为读取仓库 `AGENTS.md`、`.trellis/spec/index.md`、部署/API 边界和 Smoke 规范，并将缺失基础设施记录于此后继续恢复。
- 2026-08-15：恢复 PR #224 的页面 Smoke 为 207 通过、13 失败，其中 12 项与未包含本次改动的 PR #223 失败清单逐项相同，新增 1 项是未改动练习板页面的语言状态非确定性失败；质量审计、重构门禁、历史页与首页 Smoke 均通过。本次运行时代码仅修改 Nginx/部署链路，等待修复 13 项 UI 基线会继续延长已确认的生产 5xx。采用最保守应急偏离：不删除断言、不伪造绿色门禁、不夹带 UI 修复；保留失败证据并继续合并最小网络恢复，部署仍执行 `verify:release`，且新探针必须在 `/health`=200、匿名 Admin=404 后才接受新版本。
