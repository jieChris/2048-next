...
| T03 | 回填旧整库数据并拥有唯一兼容投影 | API | T02 |
| T04 | 实现逐套读取、bootstrap、选择和顺序 | API | T02, T03 |
| T05 | 实现内部版本历史、diff、merge 和删除原语 | API | T02, T04 |
| T06 | 实现全部外部写事务、容量、幂等和冲突副本 | API | T05 |
| T07 | 实现 Web 登录会话首同步和设置页延迟加载 | Web | T01, T04 |
| T08 | 实现编辑草稿、保存按钮、离线队列和离开保护 | Web | T07, T06 |
