...
> 状态：用户已批准本计划并启动实施；生产部署、生产迁移、正式签名、Play 上传与公开发布仍需各自单独授权。
...
- [ ] 阅读 `.trellis/spec/cross-repo-architecture.md`、`frontend-api-boundary.md`、`smoke-testing.md` 与仓库 `AGENTS.md`。
- [ ] 分别记录 `2048-next` 与 `2048-game-api` 的基线 commit、`git status --porcelain`、现有脏文件和不得触碰的并行改动。
- [ ] 两个仓库分别从已记录的基线 commit 建立全新干净 worktree；记录 worktree 绝对路径、分支和初始 `git status --porcelain`。不得在当前脏目录通过普通切分支冒充隔离；任一仓库无法安全建立干净 worktree 时停止并请求用户处理。
- [ ] 将本任务的 `prd.md`、`design.md`、`implement.md`、`execution-notes.md` 通过仅包含这四个文件的专用 patch 或规划 commit 带入新 worktree，并核对文件 hash；不得把当前工作树的其他未提交改动一并带入或暂存。
- [ ] 按项目 ADR Gate 新建一份合并 ADR，记录独立 App 构建、Game Session seam、IndexedDB、Keystore、Capacitor 依赖、回滚与验证；不拆成多份重复文档。
- [ ] 固定工具基线：Node 22、JDK 21、Android SDK 36、API 29 ARM64 system image、`ANDROID_HOME`。
...
- [ ] 首次启动单页隐私选择；“仅离线体验”不构造 HTTP 模块。
- [ ] 实现四项底部导航和首页本地主操作。
- [ ] 实现模式页三个模式卡及登录门槛，但本阶段只开放游客标准 4×4。
- [ ] 实现专属棋盘、Pointer Events、transform/opacity 动画和非阻塞 2048 里程碑。
- [ ] 实现返回保存、首页继续、对局内重新开始确认。
- [ ] 实现无撤回终局、最小结算页、本地游客历史、详情和回放。
- [ ] 结算页“再来一局 / 查看回放 / 返回首页”不依赖网络。
- [ ] 实现游客历史二次确认后永久删除。
- [ ] Playwright 拦截所有业务请求，断言离线体验完成整局仍为零请求。
...
- [x] 首屏只包含 shell、首页和必要 token；BGM、历史、榜单、成就和回放按需加载。
...
- [x] 建立 dist-app 首屏 JS/CSS、APK/AAB 和 BGM 非首屏资源预算。
