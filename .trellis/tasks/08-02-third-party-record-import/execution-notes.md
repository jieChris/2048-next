...
- 2026-08-02：Next PR 首轮 CI 的全量页面 Smoke 在既有 `pages-replay-lock` 英文提示用例失败；本 PR 未修改锁或语言运行时，本地对该用例连续重复 30 次复现同样竞态 1 次（29 通过、1 失败）。证据显示测试只向首个 `page` 注入英文设置，第二个 `context.newPage()` 偶发在跨页存储同步前启动并使用中文。采用保守回退：不在导入功能 PR 中夹带无关产品代码修复，记录证据后重新执行一次完整 CI；若同一失败重复出现，再单独审批并将测试改为 context 级初始化。
